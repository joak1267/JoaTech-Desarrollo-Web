...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class CommonClass {
      ...
      container: HTMLElement | null = null; renderer: THREE.WebGLRenderer | null = null; clock: THREE.Clock | null = null;
    ...
    class MouseClass {
      ...
      timer: number | null = null; container: HTMLElement | null = null; docTarget: Document | null = null;
    ...
    class AutoDriver {
      mouse: MouseClass; manager: WebGLManager; enabled: boolean; speed: number; resumeDelay: number; rampDurationMs: number;
      active = false; current = new THREE.Vector2(0, 0); target = new THREE.Vector2(); lastTime = performance.now();
      activationTime = 0; margin = 0.2; private _tmpDir = new THREE.Vector2();
      ...
      update() {
        ...
        this.mouse.isAutoActive = true; let dtSec = (now - this.lastTime) / 1000; this.lastTime = now;
        if (dtSec > 0.2) dtSec = 0.016; const dir = this._tmpDir.subVectors(this.target, this.current); const dist = dir.length();
    ...
    class ShaderPass {
      ...
      init(..._args: any[]) {
        ...
        if (this.uniforms) {
          ...
          this.geometry = new THREE.PlaneGeometry(2, 2); this.plane = new THREE.Mesh(this.geometry, this.material);
          this.scene.add(this.plane);
        }
      }
      update(..._args: any[]) {
        if (!Common.renderer || !this.scene || !this.camera) return;
        Common.renderer.setRenderTarget(this.props.output || null); Common.renderer.render(this.scene, this.camera); Common.renderer.setRenderTarget(null);
      }
    }
    ...
    class Advection extends ShaderPass {
      ...
      createBoundary() {
        const boundaryG = new THREE.BufferGeometry(); const vertices_boundary = new Float32Array([-1, -1, 0, -1, 1, 0, -1, 1, 0, 1, 1, 0, 1, 1, 0, 1, -1, 0, 1, -1, 0, -1, -1, 0]);
      ...
      update(...args: any[]) {
        const { dt, isBounce, BFECC } = (args[0] || {}) as any; if (!this.uniforms) return;
        if (typeof dt === 'number') this.uniforms.dt.value = dt; if (typeof isBounce === 'boolean') this.line.visible = isBounce; if (typeof BFECC === 'boolean') this.uniforms.isBFECC.value = BFECC;
        super.update();
      }
    }
    ...
    class ExternalForce extends ShaderPass {
      ...
      init(simProps: any) {
        super.init(); const mouseG = new THREE.PlaneGeometry(1, 1);
      ...
      update(...args: any[]) {
        const props = args[0] || {}; const forceX = (Mouse.diff.x / 2) * (props.mouse_force || 0); const forceY = (Mouse.diff.y / 2) * (props.mouse_force || 0);
        const cellScale = props.cellScale || { x: 1, y: 1 }; const cursorSize = props.cursor_size || 0;
        const centerX = Math.min(Math.max(Mouse.coords.x, -1 + cursorSize * cellScale.x + cellScale.x * 2), 1 - cursorSize * cellScale.x - cellScale.x * 2);
        const centerY = Math.min(Math.max(Mouse.coords.y, -1 + cursorSize * cellScale.y + cellScale.y * 2), 1 - cursorSize * cellScale.y - cellScale.y * 2);
        const uniforms = (this.mouse.material as THREE.RawShaderMaterial).uniforms;
        uniforms.force.value.set(forceX, forceY); uniforms.center.value.set(centerX, centerY); uniforms.scale.value.set(cursorSize, cursorSize); super.update();
      }
    }
    ...
    class Viscous extends ShaderPass {
      ...
      update(...args: any[]) {
        const { viscous, iterations, dt } = (args[0] || {}) as any; if (!this.uniforms) return; let fbo_in: any, fbo_out: any; if (typeof viscous === 'number') this.uniforms.v.value = viscous;
        ...
        for (let i = 0; i < iter; i++) {
          ...
          this.uniforms.velocity_new.value = fbo_in.texture; this.props.output = fbo_out; if (typeof dt === 'number') this.uniforms.dt.value = dt; super.update();
        }
        return fbo_out;
      }
    }
    ...
    class Divergence extends ShaderPass {
      ...
      update(...args: any[]) { if (this.uniforms && args[0]?.vel) this.uniforms.velocity.value = args[0].vel.texture; super.update(); }
    }
    ...
    class Poisson extends ShaderPass {
      ...
      update(...args: any[]) {
        const { iterations } = (args[0] || {}) as any; let p_in: any, p_out: any; const iter = iterations ?? 0;
        for (let i = 0; i < iter; i++) {
    ...
    class Pressure extends ShaderPass {
      ...
      update(...args: any[]) { if (this.uniforms && args[0]?.vel && args[0]?.pressure) { this.uniforms.velocity.value = args[0].vel.texture; this.uniforms.pressure.value = args[0].pressure.texture; } super.update(); }
    }
    ...
    class Simulation {
      ...
      update() {
        ...
        let vel: any = this.fbos.vel_1; if (this.options.isViscous) vel = this.viscous.update({ viscous: this.options.viscous, iterations: this.options.iterations_viscous, dt: this.options.dt });
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      dispose() {
        try {
          window.removeEventListener('resize', this._resize); if (this._onVisibility) document.removeEventListener('visibilitychange', this._onVisibility); Mouse.dispose();
    ...
    const applyOptionsFromProps = () => {
      if (!webglRef.current) return; const sim = webglRef.current.output?.simulation; if (!sim) return; const prevRes = sim.options.resolution;
