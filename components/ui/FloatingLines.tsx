...
const vec3 LIGHT_BG = vec3(0.043, 0.039, 0.051);    // Fondo oscuro premium #0b0a0d
const vec3 ACCENT_BLUE = vec3(0.0, 0.0, 0.0);         // Negro para contraste de mezcla
const vec3 DARK_BLUE = vec3(0.114, 0.184, 0.471);    // Azul JoaTech #2F4BC0
const vec3 ACCENT_PURPLE = vec3(0.573, 0.165, 0.604); // Fucsia JoaTech #E945F5
...
export default function FloatingLines({
  ...
  // Decompose complex objects/arrays to stable primitives for the useEffect dependency array
  const linesGradientStr = linesGradient ? JSON.stringify(linesGradient) : '';
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      topWavePosition: {
        value: new Vector3(topWaveX, topWaveY, topWaveRotate)
      },
      middleWavePosition: {
        value: new Vector3(middleWaveX, middleWaveY, middleWaveRotate)
      },
      bottomWavePosition: {
        value: new Vector3(bottomWaveX, bottomWaveY, bottomWaveRotate)
      },

      iMouse: { value: new Vector2(-1000, -1000) },
      interactive: { value: isInteractive },
    ...
    if (linesGradientStr) {
      const parsedStops = JSON.parse(linesGradientStr) as string[];
      const stops = parsedStops.slice(0, MAX_GRADIENT_STOPS);
